...
//returns an element with an id/sting
    ...
    if (tech_cmu462) {
      ...
      while (e_bsdf) {
        ...
        */
          ...
          float roughness = (float) atof(e_roughness->GetText());
          float ior = (float) atof(e_ior->GetText());
          ...
          XMLElement *e_ior = get_element(e_bsdf, "ior");
          float g = (float) atof(e_g->GetText());
          float ior = (float) atof(e_ior->GetText());
          Spectrum sigma_a_spectrum = spectrum_from_string(string(e_sigma_a->GetText()));
          Spectrum sigma_s_spectrum = spectrum_from_string(string(e_sigma_s->GetText()));
          BSSRDF* bssrdf = new BSSRDF(ior, sigma_a_spectrum, sigma_s_spectrum, g);
          material.bssrdf = bssrdf;
          XMLElement* e_diffuse = get_element(e_bsdf, "diffuse");
          Spectrum albedo = spectrum_from_string(string(e_diffuse->GetText()));
          material.bsdf = new DiffuseBSDF(albedo);

        } else if (type == "bssrdf-glass") {
          XMLElement *e_transmittance  = get_element(e_bsdf, "transmittance");
          XMLElement *e_reflectance  = get_element(e_bsdf, "reflectance");
          XMLElement *e_roughness = get_element(e_bsdf, "roughness");
          XMLElement *e_ior = get_element(e_bsdf, "ior");
          ...
          BSSRDF* bssrdf = new BSSRDF(ior, sigma_a_spectrum, sigma_s_spectrum, g);
          material.bssrdf = bssrdf;
        } else if (type == "bssrdf-refraction"){
          XMLElement *e_transmittance  = get_element(e_bsdf, "transmittance");
          XMLElement *e_roughness = get_element(e_bsdf, "roughness");
          XMLElement *e_ior = get_element(e_bsdf, "ior");
          Spectrum transmittance = spectrum_from_string(string(e_transmittance->GetText()));
          ...
          float g = (float) atof(e_g->GetText());
          float ior = (float) atof(e_ior->GetText());
          float sigma_a = (float) atof(e_sigma_a->GetText());
          float sigma_s = (float) atof(e_sigma_s->GetText());
          BSSRDF* bssrdf = new BSSRDF(ior, sigma_a, sigma_s, g);
          material.bssrdf = bssrdf;
        } else if (type == "bssrdf-mirror"){
          XMLElement *e_reflectance  = get_element(e_bsdf, "reflectance");
          Spectrum reflectance = spectrum_from_string(string(e_reflectance->GetText()));
          BSDF* bsdf = new MirrorBSDF(reflectance);
          material.bsdf = bsdf;
