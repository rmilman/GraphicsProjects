...
Spectrum PathTracer::raytrace_pixel(size_t x, size_t y) {
  ...
  int total_rays_tried = 0;
  Spectrum s;
  for (int i = 0; i < num_samples; ++i) {
    int rays_tried = 1;
    double cos4_term = 1;
    Vector2D p = origin + (ns_aa>1?gridSampler->get_sample():Vector2D(.5,.5));
    Ray r = camera->generate_ray(p.x/sampleBuffer.w, p.y/sampleBuffer.h, rays_tried, cos4_term);
    r.depth = max_ray_depth;
    if (cos4_term == 0) { 
    }else{
      s += cos4_term*trace_ray(r,true); //cos4_term
    }
    total_rays_tried +=rays_tried;
  }

  return s * (1./total_rays_tried); //totalraystried ns_aa
