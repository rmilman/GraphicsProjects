...
namespace CGL {
     ...
     */
    inline Halfedge* elementAddress(HalfedgeIter h) {
        return &(*h);
    }

    inline Vertex* elementAddress(VertexIter v) {
        return &(*v);
    }

    inline Edge* elementAddress(EdgeIter e) {
        return &(*e);
    }

    inline Face* elementAddress(FaceIter f) {
        return &(*f);
    }
     ...
     */
    inline Halfedge const* elementAddress(HalfedgeCIter h) {
        return &(*h);
    }

    inline Vertex const* elementAddress(VertexCIter v) {
        return &(*v);
    }

    inline Edge const* elementAddress(EdgeCIter e) {
        return &(*e);
    }

    inline Face const* elementAddress(FaceCIter f) {
        return &(*f);
    }
    ...
    public:
        ...
        HalfedgeIter& twin(void) {
            return _twin;
        } ///< access the twin half edge

        HalfedgeIter& next(void) {
            return _next;
        } ///< access the next half edge
    ...
    public:
         ...
         */
        HalfedgeCIter halfedge(void) const {
            return _halfedge;
        }
    ...
    public:
        ...
        Size nEdges(void) const {
            return edges.size();
        } ///< get the number of edges

        Size nFaces(void) const {
            return faces.size();
         ...
         */
        HalfedgeIter halfedgesBegin(void) {
            return halfedges.begin();
        }

        HalfedgeCIter halfedgesBegin(void) const {
            return halfedges.begin();
        }

        HalfedgeIter halfedgesEnd(void) {
            return halfedges.end();
        }

        HalfedgeCIter halfedgesEnd(void) const {
            return halfedges.end();
        }

        VertexIter verticesBegin(void) {
            return vertices.begin();
        }

        VertexCIter verticesBegin(void) const {
            return vertices.begin();
        }

        VertexIter verticesEnd(void) {
            return vertices.end();
        }

        VertexCIter verticesEnd(void) const {
            return vertices.end();
        }

        EdgeIter edgesBegin(void) {
            return edges.begin();
        }

        EdgeCIter edgesBegin(void) const {
            return edges.begin();
        }

        EdgeIter edgesEnd(void) {
            return edges.end();
        }

        EdgeCIter edgesEnd(void) const {
            return edges.end();
        }

        FaceIter facesBegin(void) {
            return faces.begin();
        }

        FaceCIter facesBegin(void) const {
            return faces.begin();
        }

        FaceIter facesEnd(void) {
            return faces.end();
        }

        FaceCIter facesEnd(void) const {
            return faces.end();
        }

        FaceIter boundariesBegin(void) {
            return boundaries.begin();
        }

        FaceCIter boundariesBegin(void) const {
            return boundaries.begin();
        }

        FaceIter boundariesEnd(void) {
            return boundaries.end();
        }

        FaceCIter boundariesEnd(void) const {
            return boundaries.end();
        }
        ...
        EdgeIter newEdge(void) {
            return edges.insert(edges.end(), Edge());
        }

        FaceIter newFace(void) {
            return faces.insert(faces.end(), Face(false));
        }
         ...
         */
        void deleteHalfedge(HalfedgeIter h) {
            halfedges.erase(h);
        }

        void deleteVertex(VertexIter v) {
            vertices.erase(v);
        }

        void deleteEdge(EdgeIter e) {
            edges.erase(e);
        }
        ...
        void deleteBoundary(FaceIter b) {
            boundaries.erase(b);
        }
    ...
    // inline list<Vertex> HalfedgeElement::getVertices(void) {
    //      return dynamic_cast<list<Vertex>*> (this);
    // }
